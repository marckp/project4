...
used. In the first case one can employ what is called free ends. In systems with large numbers of elements, the ends contribute a diminishingly small effect on the entire system. However, since I studied small systems, I used   
...
This quantity defines also the susceptibility $\chi$ which I used to calculate $\chi$:
...
All spins are either up or down. An up spin is given the value +1, and a down spin is given the value -1. The configurations are organized into permutations of like spin combinations (all up, all down, 1up and 3 down and its complement, 2 up and 2 down).
...
\]
which after inserting the values for $E_i$ above becomes:
\[
Z = 4 cosh(\frac{8}{T})+12
\]
\paragraph{computing the mean energy $\langle E \rangle$}
\[
...
\]
\paragraph{computing the mean square energy $\langle E^2 \rangle$}
\[
\langle E^2 \rangle = \frac{\sum_{1}^{16} E_i^2 e^{-E_i/T}}{Z}
\]
\[
\langle E^2 \rangle = \frac{ 64e^{-E_i/T} + 64e^{-E_i/T} + 64e^{E_i/T} + 64e^{E_i/T}}{Z}
\]
\[
\langle E^2 \rangle = \frac{ 256cosh(8/T)}{Z}
\]
\[
\langle E^2 \rangle = \frac{64cosh(8/T)}{cosh(8/T)+3}
\]
\paragraph{computing the mean magnetization}
\[
\langle M \rangle = \frac{1}{Z} \sum_{1}^{16} |M_i| e^{-\frac{E_i}{T}}
\]
\[
M_i = \pm \frac{1}{2}  \Rightarrow |M_i| = \frac{1}{2}
\]
\[
\langle M \rangle = \frac{1}{Z} \sum_{1}^{16} \frac{1}{2} e^{-\frac{E_i}{T}}
\]
\[
\langle M \rangle = \frac{1}{2} \frac{1}{Z} \sum_{1}^{16}  e^{-\frac{E_i}{T}} = \frac{1}{2} \frac{1}{Z} Z
\]
\[
\langle M \rangle = \frac{1}{2} 
\]
\paragraph{computing the mean square magnetization}
\[
\langle M^2 \rangle = \frac{1}{Z} \sum_{1}^{16} M_i^2 e^{-\frac{E_i}{T}}
\]
\[
M_i = \pm \frac{1}{2}  \Rightarrow M_i^2 = (\pm\frac{1}{2})^2=\frac{1}{4}
\]
\[
...
\]
\paragraph{computing the Specific Heat at constant volume}
\[
C_V = \frac{1}{k_B T^2}(\langle  E^2 \rangle - {\langle  E  \rangle}^2)
\]
\[
C_V = (\frac{64}{T^2})\frac{(1+3\cosh(8/T))}{(\cosh(8/T)+3)}
\]
\paragraph{computing the magnetic suseptability}
\[
\chi = \frac{1}{k_B T^2}(\langle  M^2 \rangle - {\langle  M  \rangle}^2)
\]
\[
